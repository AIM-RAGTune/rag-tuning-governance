...
- Synthesis: `GEN_LLM_SYNTHESIS_MIXED` because the primary CRAG slice produced bounded local generative support, independent CRAG repeats did not reproduce it, and HotpotQA remained inconclusive.
...
This supports a bounded local CRAG generated-governance cost-reduction result on one deterministic slice, plus a three-repeat non-replication pattern and a HotpotQA generated-answer quality-signal audit. It does not establish production readiness, official platform benchmarking, human validation, broad governance superiority, or RAG Compass superiority.

The current generative result is small-sample local evidence and is not stable across deterministic CRAG repeats. Additional offsets, larger samples, and human/platform validation remain future work.
