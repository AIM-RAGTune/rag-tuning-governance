...
- Generative LLM Validation v1.1 adds bounded local HotpotQA and CRAG quality-signal audits with sanitized metrics. The primary CRAG generative slice with Ollama `qwen3:8b` supports cost reduction at equivalent generated-answer quality, but three independent deterministic CRAG repeats did not reproduce it. Ollama `gpt-oss:20b` produced usable generated-quality signals on the same four offsets but no positive cost-result slices. Ollama `llama3.2:3b` repaired answer emission on larger fixed-offset slices with 0 / 704 parse failures. A predeclared latency-endpoint selector comparison with `llama3.2:3b` separated governed and quality-only winners, but produced `CRAG_GEN_LLM_LATENCY_RESULT_MIXED_ACROSS_REPEATS`: one of four slices met generated-quality noninferiority and three were quality-loss. HotpotQA remains inconclusive; the synthesis is mixed and does not move the program to Level 7.
