...
The fresh live CRAG + HotpotQA phase adds the next validation harness. Fresh CRAG remains blocked as `FRESH_CRAG_BLOCKED_MOCK_API_NOT_AVAILABLE`: approved local CRAG root/data paths and the local evaluator were configured, but the mock-API KG data were not readable by the runtime. HotpotQA ran from local cached public data with sanitized outputs and returned `HOTPOTQA_GOVERNANCE_OPERATIONAL_GAIN_QUALITY_LOSS`, meaning operating cost improved but quality noninferiority was not established. This negative/mixed status is preserved to avoid upgrading frozen-observation evidence into a replication claim.
