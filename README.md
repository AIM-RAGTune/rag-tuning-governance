...
The CRAG stability comparison is now `CRAG_GEN_LLM_LATENCY_RESULT_INCONCLUSIVE_ACROSS_REPEATS` after adding a quality-risk guardrail to the latency selector. A second pinned local generator, Ollama `gpt-oss:20b`, was also run on offsets 0, 24, 36, and 60; it produced usable generated-quality signals but no positive cost-at-equivalent-generated-quality slices and had high blank-answer rates. A faster non-thinking instruct model, Ollama `llama3.2:3b`, repaired answer emission on slightly larger 16-example fixed-offset slices. The unguarded latency selector separated governed and quality-only winners and produced one latency-positive slice plus three quality-loss slices. The guarded selector then selected `quality_guarded_latency_adaptive_expansion` against quality-only `expanded_retrieval_multi_endpoint` on the same fixed offsets, produced 0 / 768 parse failures, and avoided quality-loss labels, but it did not produce a latency-reduction CI below zero on any slice. The synthesis remains `GEN_LLM_SYNTHESIS_MIXED`. Raw prompts, raw generated answers, raw dataset questions, raw contexts, raw evidence, raw API responses, and secrets are not committed. See `docs/generative_llm_validation.md`, `docs/generator_configuration.md`, and `results/generative_llm_validation/synthesis_report.md`.
