...
The CRAG stability comparison is `CRAG_GEN_LLM_COST_RESULT_NOT_STABLE_ACROSS_REPEATS`, and the synthesis remains `GEN_LLM_SYNTHESIS_MIXED`: the primary bounded CRAG slice supports cost reduction at equivalent generated-answer quality, but independent CRAG repeats did not replicate it and HotpotQA remains inconclusive. Raw prompts, raw generated answers, raw dataset questions, raw contexts, raw evidence, raw API responses, and secrets are not committed. See `docs/generative_llm_validation.md`, `docs/generator_configuration.md`, and `results/generative_llm_validation/synthesis_report.md`.
