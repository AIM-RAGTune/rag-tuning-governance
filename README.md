...
The CRAG stability comparison is `CRAG_GEN_LLM_COST_RESULT_NOT_STABLE_ACROSS_REPEATS`. A second pinned local generator, Ollama `gpt-oss:20b`, was also run on offsets 0, 24, 36, and 60; it produced usable generated-quality signals but no positive cost-at-equivalent-generated-quality slices. The cross-model comparison is `CRAG_GEN_LLM_COST_RESULT_MIXED_OR_INCONCLUSIVE_ACROSS_MODELS`, and the synthesis remains `GEN_LLM_SYNTHESIS_MIXED`. Raw prompts, raw generated answers, raw dataset questions, raw contexts, raw evidence, raw API responses, and secrets are not committed. See `docs/generative_llm_validation.md`, `docs/generator_configuration.md`, and `results/generative_llm_validation/synthesis_report.md`.
