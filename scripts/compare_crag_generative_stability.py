...
def classify(rows: list[dict[str, object]]) -> tuple[str, str]:
    ...
    positives = [row for row in usable if positive_cost_result(row)]
    repeats = [row for row in usable if row.get("sample_offset", 0) != 0]
    positive_repeats = [row for row in repeats if positive_cost_result(row)]
    if not usable:
    ...
    if len(usable) == len(rows) and len(positives) == len(rows) and len(rows) >= 3:
        return "CRAG_GEN_LLM_COST_RESULT_STABLE_ACROSS_REPEATS", "Every available CRAG generative slice supported cost reduction at equivalent generated quality."
    if positives and positive_repeats and len(positives) < len(usable):
        return "CRAG_GEN_LLM_COST_RESULT_MIXED_ACROSS_REPEATS", "At least one independent CRAG repeat reproduced the cost result, but at least one usable slice did not."
    if positive_cost_result(rows[0]) and not positive_repeats:
        return "CRAG_GEN_LLM_COST_RESULT_NOT_STABLE_ACROSS_REPEATS", "The primary CRAG slice was positive, but no independent usable repeat reproduced the cost result."
    if positives:
        return "CRAG_GEN_LLM_COST_RESULT_DIRECTIONAL_BUT_UNSTABLE", "Some CRAG generative slices were positive, but the pattern was not stable across repeats."
    return "CRAG_GEN_LLM_COST_RESULT_INCONCLUSIVE_ACROSS_REPEATS", "Usable CRAG generative slices did not support a stable cost-at-equivalent-generated-quality result."
...
def main() -> None:
    ...
    positive_count = sum(1 for payload in payloads if positive_cost_result(payload))
    ...
    summary = {
        ...
        "positive_cost_result_count": positive_count,
...
Positive cost-at-equivalent-generated-quality runs: `{positive_count}`  
